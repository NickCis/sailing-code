...
See [sphinxbase.yaml example]({{ url }}/example/sphinxbase.yaml)
...
See [sphinxbase.spec example]({{ page.url }}/example/sphinxbase.spec)
...
Make sure that the "MerSDK" Virtual Box machine is running, since the cross compilation is done inside the Virtual Box. You can start it as explained in [Develop without qt-creator]({{ page.url }}/2015/01/01/develop-without-qtcreator/)
